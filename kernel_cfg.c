/*
 *  kernel_cfg.c
 *  Mon Aug 15 09:45:23 2016
 *  SG Version 2.00
 *  sg.exe ./sample_01_OK.oil -os=ECC2 -I../../nxtOSEK/ecrobot/../toppers_osek/sg/impl_oil -template=../../nxtOSEK/ecrobot/../toppers_osek/sg/lego_nxt.sgt
 */
...
const TaskType Task_Standing = 1;

extern void TASKNAME( Task_Main )( void );
extern void TASKNAME( Task_Standing )( void );

static __STK_UNIT _stack_Task_Main[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_Task_Standing[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 3, TPRI_MINTASK + 4, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 3, TPRI_MINTASK + 4, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000000, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( Task_Main ), TASKNAME( Task_Standing ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_Task_Main, (__STK_UNIT)_stack_Task_Standing, };
...
const AlarmType Alarm_Main = 0;
const AlarmType Alarm_Standing = 1;

DeclareTask(Task_Main);
static void _activate_alarm_Alarm_Main( void );
static void _activate_alarm_Alarm_Main( void )
{ (void)ActivateTask( Task_Main ); }

DeclareTask(Task_Standing);
static void _activate_alarm_Alarm_Standing( void );
static void _activate_alarm_Alarm_Standing( void )
{ (void)ActivateTask( Task_Standing ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_Alarm_Main, _activate_alarm_Alarm_Standing, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000001, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 1, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 40, 4, };
